...
mkdir -p build/libs
if [ ! -f "build/libs/shizuku-api.jar" ] || [ ! -f "build/libs/shizuku-provider.jar" ] || [ ! -f "build/libs/shizuku-aidl.jar" ] || [ ! -f "build/libs/shizuku-shared.jar" ]; then
    echo -e "  [📥] Downloading Shizuku API libraries (api, provider, aidl, shared)..."
    ...
    curl -sSL -o build/libs/aidl.aar "https://repo1.maven.org/maven2/dev/rikka/shizuku/aidl/13.1.5/aidl-13.1.5.aar"
    curl -sSL -o build/libs/shared.aar "https://repo1.maven.org/maven2/dev/rikka/shizuku/shared/13.1.5/shared-13.1.5.aar"
    ...
    unzip -p build/libs/aidl.aar classes.jar > build/libs/shizuku-aidl.jar
    unzip -p build/libs/shared.aar classes.jar > build/libs/shizuku-shared.jar
...
if command -v javac >/dev/null 2>&1 && [ -f "$ANDROID_JAR" ] && [ -n "$JAVA_FILES" ]; then
    ...
    javac -encoding UTF-8 -source 1.8 -target 1.8 -cp "$ANDROID_JAR:build/libs/shizuku-api.jar:build/libs/shizuku-provider.jar:build/libs/shizuku-aidl.jar:build/libs/shizuku-shared.jar" -d build/obj $JAVA_FILES
fi
...
if [ -n "$CLASS_FILES" ]; then
    ...
    if [ "$D8_CMD" = "d8" ]; then
        ...
        d8 --lib "$ANDROID_JAR" --output build/dex $CLASS_FILES build/libs/shizuku-api.jar build/libs/shizuku-provider.jar build/libs/shizuku-aidl.jar build/libs/shizuku-shared.jar
    elif [ "$D8_CMD" = "dx" ]; then
        echo -e "  [✔] Converting bytecode to classes.dex using dx..."
        dx --dex --output=build/dex/classes.dex build/obj build/libs/shizuku-api.jar build/libs/shizuku-provider.jar build/libs/shizuku-aidl.jar build/libs/shizuku-shared.jar
    fi
fi
